...
const CACHE_NAME = "drone-models-persistent-cache-v4";
const CACHE_PREFIX = "https://drone-storage.internal/models/";
const DB_NAME = "DroneAvionicsModelsCache_v4";
...
export async function loadModelIndex(cacheBuster?: number): Promise<Record<string, ModelAsset>> {
  ...
  if (!cacheBuster) {
    try {
      const stored = localStorage.getItem("drone_model_index_cache");
      if (stored) {
  ...
  try {
    localStorage.setItem("drone_model_index_cache", JSON.stringify(data));
  } catch {}
