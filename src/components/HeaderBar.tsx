    ...
    <div className="header-container-wrapper">
      ...
      <header ref={headerRef} className="header-bar" id="app-header-bar">
        ...
        <div className="header-center">
          ...
          {/* View Mode Selector: 3D | 3D + 2D Split | 2D Sxema */}
          {setViewLayoutMode && (
            <>
              <div
                className="button-group view-mode-group shrink-0 flex items-center bg-slate-900/90 rounded-lg p-0.5 border border-slate-700/70 shadow-sm"
                id="header-view-mode-selector"
              >
                <button
                  type="button"
                  id="btn-header-view-3d"
                  className={`flex items-center gap-1 px-2 py-1 rounded text-xs font-medium transition-all ${
                    viewLayoutMode === "3d"
                      ? "bg-cyan-600 text-white shadow-sm"
                      : "text-slate-400 hover:text-slate-200 hover:bg-slate-800/60"
                  }`}
                  onClick={() => setViewLayoutMode("3d")}
                  title="Faqat 3D ko‘rinish [1]"
                >
                  <Box size={13} />
                  <span>3D</span>
                  <span className="text-[10px] text-cyan-200/70">1</span>
                </button>
                <button
                  type="button"
                  id="btn-header-view-split"
                  ...
                  title="3D va 2D Sxemani bir vaqtda yonma-yon ko‘rish [3]"
                >
                  <Layers size={13} />
                  <span>3D + 2D Sxema</span>
                  <span className="text-[10px] text-cyan-200/70">3</span>
                </button>
                <button
                  type="button"
                  id="btn-header-view-2d"
                  className={`flex items-center gap-1 px-2 py-1 rounded text-xs font-medium transition-all ${
                    viewLayoutMode === "2d"
                      ? "bg-cyan-600 text-white shadow-sm"
                      : "text-slate-400 hover:text-slate-200 hover:bg-slate-800/60"
                  }`}
                  onClick={() => setViewLayoutMode("2d")}
                  title="Faqat 2D Sxema (Top-down muhandislik chizmasi) [2]"
                >
                  <GitBranch size={13} />
                  <span>2D Sxema</span>
                  <span className="text-[10px] text-cyan-200/70">2</span>
                </button>
              </div>

              {/* Bitta tugma orqali 3D va 2D sahnalarini almashtirish */}
              <button
                type="button"
                id="btn-header-quick-toggle-3d-2d"
                className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-semibold bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-500 hover:to-blue-500 text-white shadow-sm border border-cyan-400/60 transition-all active:scale-95 shrink-0 cursor-pointer"
                onClick={() => setViewLayoutMode(viewLayoutMode === "3d" ? "2d" : "3d")}
                title="Bitta tugma orqali 3D va 2D sahnalarini almashtirish (Klaviatura: [Tab])"
              >
                <RotateCcw size={12} className="text-cyan-200" />
                <span>{viewLayoutMode === "3d" ? "📋 2D Sxema" : "🧊 3D Sahna"}</span>
